...
class Token extends Controller
{
    use Send;
    //手机客户端请求验证规则
    public static $rule_mobile = ['app_key' => 'require', 'user_email' => 'require', 'user_pwd' => 'require', 'nonce' => 'require', 'timestamp' => 'require'];
    //微信端请求验证规则
    public static $rule_wechat = ['app_key' => 'require', 'open_id' => 'require', 'nonce' => 'require', 'timestamp' => 'require', 'union_id' => 'require', 'access_token' => 'require'];
    /**
     * 构造函数
     * 初始化检测请求时间，签名等
     */
    ...
    {
        $this->checkAppkey(self::$rule_wechat);
        //检测appkey
    }
    /**
     * 为客户端提供access_token
    ...
    public function mobile()
    {
        ...
        if (!empty($this->request->param('captcha'))) {
            ...
            $mobilebind = Db::name('admin')->field('user_name,user_id as uid')->where('user_email', $this->request->param('user_email'))->where('user_pwd', md5($this->request->param('user_pwd')))->find();
            // dump($mobilebind);
            //取数据库对应手机号绑定用户
    ...
    public function checkSign()
    {
        ...
        $sign = $baseAuth->makesign($this->request->param(), $app_secret['app_secret']);
				// dump($this->request->param());echo $sign;exit;
        //生成签名
    ...
    protected function setAccessToken($clientInfo)
    {
        ...
        $accessTokenInfo = [
            ...
            //访问令牌
            'expires_time' => time() + Oauth2::$expires,
            //过期时间时间戳
            'client' => $clientInfo,
        ];
